...
/* ============================================================
   ...
   密钥与中英系统提示词均在 DGdubc53yfWv/judge.js（部署为 waxyl-api 这个 Worker），
   ...
   ============================================================ */
// ============================================================
//   AI 裁决：浏览器直连硅基流动（SiliconFlow），key 埋在前端。
//   CORS 全开（*），无中间层、不被墙、不依赖任何 Worker/隧道。
//   模型：tencent/Hunyuan-MT-7B（免费模型）
//   key 多层混淆：拆段→乱序→base64→XOR盐→再base64，运行时还原
// ============================================================
const _k_s = 'a/KyCBqaEFhL2klSYIHt4ZMZyhF+0J9ZRrFIb3y+F1xKl8bQ337bM2KF4xgahWNfc7guAkWHmKyeIJQFO9TsKg6vaBUX9xddb5T8ybBwxh4yiKIBFp9BcUTZDgx73Zg=';
const _k_x = [90, 60, 159, 33, 126, 68, 184, 13, 99, 46, 209, 122, 9, 245, 140, 48, 75, 22, 226, 120];
const _k_o = [3, 4, 6, 7, 2, 5, 0, 1];
const _k_n = 8;
const _k_d = (() => {
  for (let i = 0; i < _k_x.length; i++) _k_x[i] = (_k_x[i] * 3 + 1) & 0xff;
  const raw = atob(_k_s);
  let dec = '';
  for (let i = 0; i < raw.length; i++) dec += String.fromCharCode(raw.charCodeAt(i) ^ _k_x[i % _k_x.length]);
  const pieces = dec.split('|');
  const ordered = new Array(_k_n);
  for (let i = 0; i < _k_n; i++) ordered[_k_o[i]] = pieces[i];
  return ordered.map(p => atob(p)).join('');
})();
const SILICONFLOW_KEY = _k_d;
const AI_MODEL = 'tencent/Hunyuan-MT-7B';
const AI_ENDPOINT = 'https://api.siliconflow.cn/v1/chat/completions';

const AI_SYSTEM_ZH = `你是许愿柳的裁决者。用户会许下一个愿望，你要判断柳枝是否答应。
规则：
- 温情、善良、对家人朋友的守护祝愿、对自身生存健康的祝愿 → 成全，输出 good
- 投机取巧、不劳而获、一夜暴富、中彩票、想靠超能力走捷径 → 反转，输出 twist
- 长生不老、永生不死、控制他人、伤害他人、违背常理、懒惰敷衍 → 拒绝，输出 refuse
- 涉及违法、色情、暴力、政治敏感、伤害他人 → 禁止，输出 forbidden
严格只输出一行 JSON：{"result":"good|twist|refuse|forbidden","outcome":"简短的一句话判词，用中文，带一点点柳枝的神秘感"}。
outcome 不要超过 30 字。`;

const AI_SYSTEM_EN = `You are the judge of the Wish Willow. The user makes a wish; decide whether the willow grants it.
Rules:
- Warm, kind wishes for family/friends' well-being, or one's own survival/health → good
- Get-rich-quick, lottery, superpowers, shortcuts → twist
- Immortality, controlling others, harming others, lazy/vague wishes → refuse
- Illegal, pornographic, violent, politically sensitive, harming others → forbidden
Output strictly one line of JSON: {"result":"good|twist|refuse|forbidden","outcome":"short one-sentence verdict in English, with a hint of willow mystery"}.
outcome no more than 30 words.`;

async function judgeByAI(wish) {
  // 超时兜底：AI 请求挂起时 10s 后强制 abort，
  // 抛出的 AbortError 会被 judgeWish 的 catch 接住，走 fallbackJudge 本地裁决，
  // 避免"柳枝正在审视你的愿望"加载层永远卡住。
  const ctrl = new AbortController();
  const timer = setTimeout(() => ctrl.abort(), 10000);
  try {
    const system = LANG === 'en' ? AI_SYSTEM_EN : AI_SYSTEM_ZH;
    const user = LANG === 'en'
      ? `Wish: "${wish}"\n\nJudge it now. Output JSON only.`
      : `愿望：「${wish}」\n\n请裁决。只输出 JSON。`;
    ...
    const match = text.match(/\{"result"\s*:\s*"(good|twist|refuse|forbidden)"\s*,\s*"outcome"\s*:\s*"([^"]*)"\s*\}/);
    if (!match) throw new Error('parse fail: ' + text.slice(0, 80));
    return { result: match[1], outcome: match[2] };
  } finally {
    clearTimeout(timer);
  }
}
...
function fallbackJudge(wish) {
  ...
  // 生存关怀豁免：不限定亲人——"希望我能好好活着/健康平安/康复"这类对自身的
  // 生存祝愿同样判 good。放在 refuse 之后，避免"永生/不死"类被误判。
  const careAlone = /好好活着|好好活|活着|健康|平安|康复|好起来|长命|长寿|活下去|身体|不生病|少生病/;
  const careAloneEn = /\b(alive|stay alive|keep living|live long|health|healthy|safe and sound|recover|recovery|no more illness|pain free|survive)\b/i;
  if (careAlone.test(wish) || careAloneEn.test(wish)) return { result: 'good', key: 'fbCare', args: [kw, v] };
